...
mats <- Read10X(data.dir = paste0(dir_10x, "count/reads")) 
print(names(mats))
```
...
## 2.3 Filtering for cells that got 1 gRNA and adding gRNA metadata
...
```{r}
# nFeatures
p1_qlow <- quantile(x = so$nFeature_RNA, 0.05) # here we use top and bottom 5% cutoff
...
p3 <- VlnPlot(object = so, 
              ...
              cols = c("#037bfc", "#037bfc", "#037bfc")) +
  NoLegend() +
  geom_hline(yintercept = 10, linetype = "dashed", color = "red")+ # here we use 10% MT DNA cutoff
  theme(axis.text.x = element_blank(), 
        axis.ticks.x = element_blank())+
  xlab(NULL)
...
```{r}
#### Apply filter matrix here
n_before <- ncol(so)
so <- subset(so, subset = nFeature_RNA > p1_qlow & nFeature_RNA < p1_qhigh & percent.mt < 10) # here we use 10% MT DNA cutoff
...
## 7.1 Perfrom DEG of one NT guide against the rest of NT guides
       Here I am using Replogle et al. method to perfrom NTC similiar testing
       
     # 1) Subset to only contain genes with mean UMI > 1
     # 2) z-normalized the expression
     # 3) Perform Anderson-Darling test between all possible pairs of NTC

```{r}

# Subset to only contain mean gene UMI > 1
raw_counts <- GetAssayData(so_ntc, assay = "RNA", layer = "counts")
gene_means <- rowMeans(raw_counts)

# Filter for genes with mean > 1 UMI count per cell
high_umi_genes <- names(gene_means[gene_means > 1])
so_ntc_filtered <- subset(so_ntc, features = high_umi_genes)

# z-normalized the expression
so_ntc_filtered <- ScaleData(so_ntc_filtered, features = high_umi_genes)
z_data <- GetAssayData(so_ntc_filtered, layer = "scale.data")

# Identify all NTC guides
ntc_guides <- unique(so_ntc_filtered$gRNA) # adjust metadata column name as needed
pairs <- combn(ntc_guides, 2) # Get all possible pairs
...
system.time({
  ad_results_list <- future_lapply(pairs_list, function(p) {
    g1 <- p[1]
    g2 <- p[2]
    
    # Pre-extract data for these two guides to avoid subsetting 2000 times
    cells_g1 <- which(so_ntc_filtered$gRNA == g1)
    cells_g2 <- which(so_ntc_filtered$gRNA == g2)
    
    data_g1 <- z_data[, cells_g1, drop = FALSE]
    data_g2 <- z_data[, cells_g2, drop = FALSE]
    
    # Run tests for all genes in this pair
    gene_results <- lapply(high_umi_genes, function(gene) {
      res <- ad.test(data_g1[gene, ], data_g2[gene, ])
      return(data.frame(
        Gene = gene,
        Pair = paste(g1, g2, sep = "_vs_"),
        p_val = res$ad[1, 3]
      ))
    })
...
ad_results <- do.call(rbind, ad_results_list)

ad_results$p_val_adj <- p.adjust(ad_results$p_val, method = "BH")
...
## 7.2 Perform Wilcox test between one NTC and the rest of NTCs
...
deg_counts <- ntc_rest_deg %>%
  dplyr::filter(study_wide_fdr < 0.05) %>%  # apply a log fold change cutoff of 20%
...
## 7.3 Perform pair-wise DEG among all pairs of guides
...
## 7.4 PCA analysis of all NTC guides

```{r}
plan("sequential")
so_ntc <- FindVariableFeatures(so_ntc, selection.method = "vst", nfeatures = 2000)
so_ntc <- ScaleData(so_ntc)
so_ntc <- RunPCA(so_ntc, features = VariableFeatures(object = so_ntc))

DimPlot(so_ntc, reduction = "pca", group.by = "gRNA") + 
  ggtitle("PCA of NTC Cells: Grouped by Guide")
...
```{r}
# set expression matrix and define control cells
...
## 8.2 Plot expression level of target gene of all guides compared to NTC

```{r}

list_of_target_genes <- unique(meta_data$target_gene_name)
list_of_target_genes <- setdiff(list_of_target_genes, "Non-targeting")



for (target_gene in list_of_target_genes){
  
  # Get control cell expression matrix and dataframe
  vec_control <- expr_matrix[target_gene, control_cells]
  
  df_control <- data.frame(
    Expression = vec_control,
    Group = "Non-Targeting Control",
    Type = "Control"
  )
  
  guides_in_gene <- str_sort(unique(meta_data$gRNA[meta_data$target_gene_name == target_gene]), numeric = TRUE)
  guide_data_list <- list()
  guide_counts <- list()
  
  # Control cell count
  control_count <- length(vec_control)
  guide_counts[["Non-Targeting Control"]] <- control_count
  
  for (guide in guides_in_gene) {

    kd_cells <- rownames(meta_data)[meta_data$gRNA == guide]
    
    if (length(kd_cells) > 20) {
      vec_kd_cells <- expr_matrix[target_gene, kd_cells]
      
      # Create a temporary dataframe for this specific guide
      df_temp <- data.frame(
        Expression = vec_kd_cells,
        Group = guide,
        Type = "Guide"
      )
      
      # Add to our list
      guide_data_list[[guide]] <- df_temp
      
      # Add cell counts
      guide_counts[[guide]] <- length(kd_cells)
  ...
  # combine all guide information with target_gene
  df_guides_all <- do.call(rbind, guide_data_list)
  plot_data <- rbind(df_control, df_guides_all)
  
  ordered_groups <- c("Non-Targeting Control", names(guide_data_list))
  plot_data$Group <- factor(plot_data$Group, levels = ordered_groups)
  
  # Create label data
  label_data <- data.frame(
    Group = ordered_groups,
    N_Cells = unlist(guide_counts[ordered_groups])
  )
  
  
  max_y_value <- max(plot_data$Expression)
  label_data$y_pos <- max_y_value * 1.1 # Position label 5% above the max data point

  
  # Plotting
  p <- ggplot(plot_data, aes(x = Group, y = Expression, fill = Type)) +
    geom_violin(scale = "width", trim = FALSE) +
    geom_boxplot(width = 0.1, fill = "white", outlier.shape = NA) +
    geom_text(
      data = label_data, 
      aes(x = Group, y = y_pos, label = paste("N =", N_Cells)),
      inherit.aes = FALSE, # Do not inherit fill = Type
      size = 3,            # Adjust size as needed
      vjust = 0            # Vertically align the text at the bottom of y_pos
    ) +
    scale_fill_manual(values = c("Control" = "grey", "Guide" = "lightblue")) +
    labs(
      title = paste("Target Gene Expression:", target_gene),
      x = "gRNA ID",
      y = "Expression Level"
    ) +
    theme_minimal() +
   ...
   ggsave(filename = paste0("iPSC_figures_fork/target_gene_expr/", target_gene, "_gRNA_Expression.png"), plot = p, width = 4000, height = 1500, dpi = 300, units = "px")
...
## 8.3 Subset the Seurat object with only guide pass logFC filter
...
## 8.4 Using alternative method to calculate guide knockdown efficiency
       MAST - this method is slower since it requires to calculate the entire transcriptomic profile not just the target gene
...
for (guide in list_of_guides) {
  ...
  deg <- FindMarkers(so_sub,
                       ident.1 = "Knockdown",
                       ident.2 = "Control",
                       features = c(target_gene, "GAPDH"), # only test the target gene with a dummy gene otherwise it won't pass at filter
                       test.use = "MAST",
                       logfc.threshold = 0,
                       min.pct = 0.1,
                       verbose = FALSE)
...
gRNA_knockdown_summary_MAST <- do.call(rbind, guide_kd_result)
write_xlsx(gRNA_knockdown_summary_MAST, path = "iPSC_figures_fork/gRNA_knockdown_summary_MAST.xlsx")

gRNA_knockdown_summary_MAST <- gRNA_knockdown_summary_MAST %>% 
  dplyr::filter(avg_log2FC < log2(0.7) & p_val < 0.05)

# check guide left for each gene
table(gRNA_knockdown_summary_MAST$target_gene)
logFC_filter_guide_MAST <- gRNA_knockdown_summary_MAST$guide_name


# Check whether MAST vs Wilcoxon text gives the same guide filter
identical(logFC_filter_guide, logFC_filter_guide_MAST)
```
...
# 9. Run basic Wilcoxon test on guides that passed the logFC filter
     Pseudo-bulk all the guide together
     
```{r}
# load Seurat object post-logFC filter
so_logFC_filter <- qs_read(file = paste0(getwd(), "/data/2_COMPASS-iPSC-gRNA-logFC-filtered.qs2"))
meta <- so@meta.data
```
```{r}

list_of_genes <- unique (so_logFC_filter@meta.data$target_gene_name)
list_of_genes <- setdiff(list_of_genes, "Non-targeting")
list_of_degs <- list()
...
for (gene in list_of_genes) {
  ...
  print(paste0("Performing DE Analysis on ", gene))
 
   so_logFC_filter_sub <- subset(so_logFC_filter, subset = NT_gRNA == "non-targeting" | target_gene_name == gene)

  so_logFC_filter_sub$comparison_group <- ifelse(so_logFC_filter$target_gene_name == gene, "Knockdown", "Control")
  Idents(so_logFC_filter_sub) <- "comparison_group"
  
  deg <- FindMarkers(so_logFC_filter_sub,
                         ident.1 = "Knockdown",
                         ident.2 = "Control",
                         logfc.threshold = 0,
                         min.pct = 0.1, 
  ...
  # Perform FDR adjustment, default Seurat one is Bonferroni, too strigent
  deg$p_val_adj_fdr <- p.adjust(deg$p_val, method = "fdr")
  
  
  deg_filter <- deg %>% dplyr::filter(p_val_adj_fdr<0.05 & abs(avg_log2FC) > log2(1.2))
  up_deg <- nrow(deg_filter %>% dplyr::filter(avg_log2FC > 0) )
  down_deg <- nrow(deg_filter %>% dplyr::filter(avg_log2FC < 0) )
  total_deg <- nrow(deg_filter)
  gene_name <- gene
  ...
  list_of_degs[[gene]] <- df_temp
  
}

DEG_summary <- do.call(rbind, list_of_degs)

write_xlsx(DEG_summary, path = "iPSC_figures_fork/DEG_logFC_Filter_Wilcoxon.xlsx")
...
so <- CalcPerturbSig(
  ...
  assay = "RNA",
  slot = "data",
  gd.class = "target_gene_name",  #gRNA
  nt.cell.class = "Non-targeting",
  reduction = "pca",
  ndims = 15,
  num.neighbors = 20,
  new.assay.name = "PRTB",  #perturb scores by gRNA
